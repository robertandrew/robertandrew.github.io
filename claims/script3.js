...
function updateBars(startYear){
	...
	var rBind = visCon.selectAll('rect.recessions')
	...
	var cBind = visCon.selectAll('rect.claims')
...
function bars(startYear){
	...
	d3.select('g.recessions')
		.selectAll('rect')
		.data(filterRecessions, rKey)
		.enter()
		.append('rect')
	...
	//draw the jobless bars		
	d3.select('g.claims').selectAll('rect')
		.data(filterClaims, cKey)
		.enter()
		.append('rect')
		.attr('width',barWidth.rangeBand())
	...
	//declare functionwide variables for mouseover
	var tempH;
	var tempW;	
	}; //CLOSE bars
