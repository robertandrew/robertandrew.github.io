...
var tempest;
var gdp;
var mostRecentSet; //global that stores the most recent datast so that it can be called again upon resize
...
function stricterSetWidth(){
	...
	if(win > 959){w = 959 * 0.9- cPad - cPad - labels; console.log('is big')}

	else if(win <= 959 && win > 650){w = 650 * 0.9 - cPad - cPad - labels; console.log('is medium')}//650 can be changed to "win" if we want dynamic resizing within the typical window range

	else if(win <= 650 ){w = 350 * 0.9 - cPad - cPad - labels; console.log('is small')}
	console.log("that one variable is " + w);
	};
...
d3.tsv('gdpq2.tsv',function(error,data){
	...
	var oldWin = d3.round(w,0); //compares the new resize window to see if we've crossed a Rubicon
	window.addEventListener('resize',function(event){
		oldWin = d3.round(w,0);// this is before we rerun setWidth		
		stricterSetWidth();
		console.log(w + " and the old one is " + oldWin);
		if(w != oldWin){
			//call functions if the window has been resized a lot
			nav();	
			bars(mostRecentSet);
			console.log('resized because ' + oldWin + ' is different than ' + w);
