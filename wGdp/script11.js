...
function nav(){
	...
	var navGEnter = navG.enter()
		.append('g')
		.attr('class','navBars');
	
	var navGUpdate	
	
	var groupedRect = navG.selectAll('rect')
		.data(function(d){return d});
		
	groupedRect.enter()
		.append('rect');
	
	groupedRect.attr('x',function(d,i){return navX(d.date)})
		.attr('y',function(d,i){
			if(d.value <= 0){
				return (navY(0))
	...
	var yGroup = navCon.selectAll('text#years')
		.data(yRange,function(d){return d.date});
	
	var yearEnter = yGroup.enter()
		.append('text');
