...
var w = 500 - labels -cPad - cPad // (labels * -1) + window.innerWidth - cPad *2;
...
//		.call(navXaxis)
	...
	//stretch the tickts out into horizontal lines
	
	d3.selectAll('g#navY .tick line ')
		.attr('x2',w + labels + navBarWidth.rangeBand())
...
/////Waterfall mouseovers
		...
		barEnter.on('mouseover',function(d){
			...
			.on('mouseout',function(d){
				...
				d3.select(this).style('opacity',prevPacity);				
				d3.select('text#' + d.key)
					.transition()
					.duration(duration)
					.style('font-weight',prevWeight)
					.attr('x',prevX);
				})	
