...
function changeBars(startYear,endYear){
	...
	//revise scales as necessary
	scalex.domain([0,winSize * 0.7]);
	
//	scalex.range([0,d3.max(eiHarmonize, function(d){return d.value - d[0].value;})])
	...
	var barCon = d3.select('div.viz')
		...
		svg.append('rect')
			...
			.attr('width', function(d,i){
				...
				var monthText = d.values.map(function(d){return d.month});
				var setLength = d.values.length;								
				
				var monthTranslator = d3.scale.ordinal()
					.domain(monthText)					
					.range(d3.range(setLength),0);
				...
				console.log(startMo + " to " + endMo + " begets " + d3.round((pctChg(startVal,endVal)),1) + "%");	
