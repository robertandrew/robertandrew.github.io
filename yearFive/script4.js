...
function dataImport(tsv, hedder){
	...
	d3.tsv(tsv + '.tsv',function(error, vData){
		...
		var svg = boxer.append('svg')
			.attr('width',chartWidth + cPad + cPad)
		...
		lineBox.append('rect')
			...
			.attr('x',0)
			.attr('y',0)
			.attr('transform','translate(' + (cPad * -0.5) + ',' + (-1 * cPad) + ')')
		...
		lineBox.append('text').attr('class','dek')
			...
			.attr('y',0)
			.text(function(d){
				return 'Beginning June of ' + d.key.substring(1,5)})
				.attr('transform','translate(' + (cPad * -0.4) + ',-5)');
...
d3.tsv('years.tsv',function(error, yData){
	yData.forEach(function(d){
		...
		dataImport('sectors', 'change in payrolls by sector');
	});//CLOSE years.tsv
