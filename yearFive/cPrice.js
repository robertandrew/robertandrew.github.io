//Show the prices of everything as bars, with the option to add in year and sort by year as well.
...
var priceLabels = d3.scale.ordinal();	
...
function dataImport(tsv, hedder){
	...
	d3.tsv(tsv + '.tsv',function(error, vData){
		...
		var keySpan = keyDiv.append('span').attr('class','buttOn')
		...
		keyDiv.append('span')
			.text(function(d1){
				...
				if(tsv != 'prices'){
					return d1.key}
				if(tsv == 'prices'){
					return priceLabels(d1.key);					
					}	})	
			.attr('class','keyLabel');
			
		
		keyDiv.on('mouseover',function(d){
			d3.select(this)
				.style('opacity',0.5);
		...
		grouper.transition()
			...
			var addText = d3.select(this).append('text')
						.attr('x',chartWidth + 5)
						.attr('y',scaleY(perc))
						.text(d3.round(perc,1) + '%')
						.attr('class','toolOff')
						.attr('dy','0.5em')
						.attr('id',d5.key);
			var addCircle = d3.select(this).append('circle')
				.attr('cx',chartWidth)
				.attr('cy',scaleY(perc))
				.attr('r',3.5)
				.attr('class','toolOff')
				.attr('id', d5.key)
				.style('fill',colorize(d5.key))	;
...
d3.tsv('years.tsv',function(error, yData){
	yData.forEach(function(d){
		...
		d3.tsv('items.tsv',function(error,iData){
			
			items = iData;
				priceLabels.domain(items.map(function(d){return d.item_code}))
				.range(items.map(function(d){return d.item_name}));
				
		dataImport('prices', 'select consumer-price indexes');
