...
function dataImport(tsv){
	...
	d3.tsv(tsv + '.tsv',function(error, vData){
		...
		barBox.selectAll('rect').data(function(d){return d.values})
			.enter()
			.append('rect')
			.attr('x',function(d2){
				var start = scaleY(d2.values[0].value);
				var end = scaleY(d2.values[d2.values.length-1].value);
				var pctg = (end - start)/start * 100;
					if(pctg < 0){
						return scaleBar(0) - scaleBar(pctg)}
					if(pctg > 0){
						return scaleBar(0);};})
			.attr('y', function(d1){return barPlace(d1.key)})
			.style('fill',function(d1){
				return colorize(d1.key);})
			.attr('height', barHeight.rangeBand())
			.attr('width', function(d2){
				
				var start = (d2.values[0].value);
				var end = (d2.values[d2.values.length-1].value);
				var pctg = (end - start)/start * 100;
				if(pctg > 0){
					return scaleBar(pctg)}
					if(pctg < 0){
						return scaleBar(pctg * -1);};
				})	
				.append('title')
				.text(function(d){});
