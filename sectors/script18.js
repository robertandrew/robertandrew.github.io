...
///////////////////////////////////		
	...
	function reBarrer(barDate){
		...
		//MONTLY CHANGE: percentage change on rounded rect 
			//height and width double lineHeight
			//font size of double lineheight, centered on the midpoint of the box
			//no plus sign or percent sign

		//full label on one line 
			//height lineHeight, font lineHeight		
		
		//SECTOR SIZE: sector size directly below it on one line
			//bar 1.0 lineheight, label directly after it
			
		//MONTHLY UP [blue pill] AND DOWN [red pill], during time period shown above
			//Two circles showing how many months it's fallen, how many risen
			//max radius of 1.2 lineHeight
		
		//gap of 1 line height
		//responsive puts circles/square on one line.
	...
	function barrer(barDate){	
		...
		console.log(upDown[2]);
		console.log(barSet[2]);		
		
		barSet.forEach(function(ebar,dbar){
			upDown.forEach(function(eUp,dUp){
				if(dbar.series == dUp.series){
					console.log(dUp.series)
					console.log(dUp.up)
					console.log(dUp.down)
					ebar.up = dUp.up;
					dbar.down = dUp.down;
