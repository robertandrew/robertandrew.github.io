(function() {
	...
	$(function() {
		...
		//Sort my nested dataset based on three parameters: 'pctChange', 'change', or 'value'
		function sorter(sortSet) { 
				if (sortCriteria == 'pctChange' || sortCriteria == 'change') {
					sortSet.forEach(function(d, i) {

							//sort each nested value in descending order based on percentage change
							d.values = d.values.sort(function(a, b) {
								return d3.descending(a[sortCriteria], b[sortCriteria])
							});
						...
						}) //close the nest sorter forEach

				} //CLOSE the change conditional
				...
				if (sortCriteria == 'value'){
					sortSet.forEach(function(d, i) {
							...
							//add the RANK for each type
							d.values.filter(function(d){return d.change > 0}).forEach(function(d, i) {				
									d.rank = i + 1;
									d.fill = posFill;							
							})
							d.values.filter(function(d){return d.change < 0}).forEach(function(d, i) {				
									d.rank = (valLength - (i + posCount + zeroCount + 1)) * -1;
									d.fill = negFill;							
							})

						}) //close the nest sorter forEach
				} //CLOSE the value conditional
		...
		function boxer(dataSetRaw, starter, recessSet) {
				...
				var dataSet = dataSetRaw.filter(function(d) {
					return d.values[0].dateObj >= starter
				});

				//constrict the recessions, but also modify the start date
				var recess = recessSet.filter(function(d) {
					return d.trough >= starter
				});
				...
				var downScaler = d3.scale.ordinal()
					.domain(upDown.map(function(d) {
						return d.series
					}))
					.range(d3.range(0, upDown.length));

				//iterate through the dataset and pull the highest absolute value for color-scaling purposes
				var topCrit = 0;
				...
				var longestSet = d3.max(dataSet, function(d, i) {
				...
				var recessDraw = d3.select('g#recessions')
					.selectAll('rect')
					.data(recess);

				recessDraw.enter().append('rect');
				//	.append('title').text(function(d) {	return d.recession	});

				recessDraw.exit().remove();

				recessDraw.attr('height', ch)
					.style('fill', neutralFill)
					.style('opacity',0.3)
					.attr('width', function(d) {
						return scaleX(d.trough) - scaleX(d.peak)
					})
					.attr('x', function(d) {
						return scaleX(d.peak)
					})
					.attr('y', 0);
				//	.attr('transform', 'translate(' + boxWidth.rangeBand() + ',0)');//scoot recessions over to compensate for the fact that recessions begin the month after the peak begins


				//Drop in SHADERS to highlight the selected month				
				var highlightShaders = d3.select('g#highlightShaders')
					.selectAll('rect')
					.data(dataSet, function(d) {
						return d.key
					});

				highlightShaders.enter()
					.append('rect')
					.attr('id', function(d) {
				...
				highlightShaders.attr('x', function(d) {
						return scaleX(parseDate(d.key))
					})
					.attr('y', 0)
					.attr('height', ch)
					.style('fill', bgFill)
				...
				var boxUpdater = boxDrawer
					...
					.attr('y', function(d) {
						return scaleY(d.rank)
					})
					.attr('rx', boxWidth.rangeBand() * 0.3)
					.attr('ry', boxWidth.rangeBand() * 0.3)
				...
				function rebarrer(barDate) {
						...
						d3.selectAll('g#highlightShaders rect')
							.style('opacity', 0)
							.attr('height', ch)
							.attr('width', boxWidth.rangeBand());
						...
						var selector = d3.scale.ordinal()
							.domain(dataSet.map(function(d) {d.values.filter(function(d){return d.change > 0}).forEach(function(d, i) {				
									d.rank = i;
									d.fill = posFill;							
							})
								return d.key
						...
						var scaleRect = d3.scale.linear()
							.domain([0, d3.max(rebarSet, function(d) {
								return d.value
							})]);

						var scaleCircle = d3.scale.sqrt()
							.domain([0, Math.max(d3.max(rebarSet, function(d) {
								return d.up
							}), d3.max(rebarSet, function(d) {
								return d.down
							}))]);
						...
						barBoxLabels
							...
							.text(function(d) {
								...
								if(sortCriteria=='pctChange'){
									return d['pctChange']								
								}
								
								if(sortCriteria=='change'){
									return d3.round(d['change'],0) + 'k';								
								}
						...
						barRectLabels
							.attr('y', function(d, i) {
								return ((i + 1) * (gh + (0.5 * lh))) + boxH * 0.25
							})
							.attr('x', function(d) {
								return scaleRect(d.value) + 5
							})
							.text(function(d) {
								return d3.round(d.value / 1000, 1) + "m"
							})
							.attr('dy', '0.4em')
							.attr('id', function(d) {
								return d.series
						...
						downCircleLabels
							...
							//LABEL THE COLUMNS
							var xOffset = $('g#shadedRect rect').offset().top;
							var yOffset = $('g#shadedRect rect').offset().left;
							
							console.log(scaleCircle.range()[1])
							
							var hedText = 'Nonfarm payrolls this month';
							var pctChangeText = 'Monthly change (pct.)';
							var changeText = 'Monthly change (thousands)';
							var sizeText = 'Overall sector size this month';
							var upCircleText = 'Months rising';
							var downCircleText = 'Months falling'; 
			
							//SET THE XY FOR ALL ALONG WITH DEFAULT TEXT
					
							d3.select('#shadedRect h5').attr('x',0).attr('y',0).text('Nonfarm payrolls for this month')
				
							d3.select('#barBoxes text').attr('class','columnLabel').attr('x',0).attr('y',lh).text('Percentage change')
							
							d3.select('#barRects text').attr('class','columnLabel').attr('x',0).attr('y',lh).text('Overall sector size')
								
							d3.select('#upCircles text').attr('class','columnLabel').attr('x',0).attr('y',lh).text('Months growing/shrinking')
				
							
							//SET THE TEXT, which could change dynamically		
							if(sortCriteria == 'pctChange'){		
								d3.select('#barBoxes text').text('Rising tides');
								
									d3.select('#barRects text').attr('class','columnLabel').attr('x',0).attr('y',lh).text('Rising tides');
								
								d3.select('#upCircles text').attr('class','columnLabel').attr('x',0).attr('y',lh).text('Rising tides');
							}//close pctChange conditional	
				...
				d3.tsv('recessions.csv', function(eRecession, dRecession) {
						...
						d3.tsv('payrolls.tsv', function(error1, data1) {
								...
								var payNest = d3.nest()
									...
									.entries(payArray.filter(function(d, i) {
										return d.value != 0
									}));
								...
								var startDate = '01/01/2008'; //this is a default that will get updated
								sorter(payNest, 'pctChange');
								...
								d3.select('button#y2k').on('click', function(d) {
									d3.selectAll('div#timeButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'but-primary');
									startDate = '01/01/2000';
									boxer(payNest, startDate, recessions);
								})

								d3.select('button#recession').on('click', function(d) {
									d3.selectAll('div#timeButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'but-primary');
								...
								d3.select('button#recovery').on('click', function(d) {
									d3.selectAll('div#timeButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'but-primary');
								...
								d3.select('button#pctChange').on('click', function(d) {
									d3.selectAll('div#sortButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'btn-primary');
									sortCriteria = 'pctChange';
									sorter(payNest);
									boxer(payNest, startDate, recessions);
								})

								d3.select('button#change').on('click', function(d) {
									d3.selectAll('div#sortButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'btn-primary');
									sortCriteria = 'change';
									sorter(payNest);
								...
								d3.select('button#value').on('click', function(d) {
									d3.selectAll('div#sortButtons button').attr('class', 'btn btn');
									d3.select(this).attr('class', 'btn-primary');
									sortCriteria = 'value';
									sorter(payNest);
