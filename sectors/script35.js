...
//GLOBALish VARIABLES
var sortCriteria = 'pctChange';//pctChange is just the default value
var shadeCriteria = 'pctChange';//pctChange is just the default value
var startDate = '12/01/2007';//this is a default that will get updated
...
///////////////////////////////////		
	...
	function rebarrer(barDate){
		...
		rebarSet.forEach(function(dbar,ibar){
			...
			dataSet.forEach(function(d,i){
				d.values.forEach(function(d,i){
						...
						var thisCrit = ab(d[shadeCriteria]);
						var rawC = d[shadeCriteria];
						if(rawC>0){
			...
			dataSet.forEach(function(d,i){
				d.values.forEach(function(d,i){
					d.boxOpacity = opacityRange(ab(d[shadeCriteria]));
					})		
				});	
		...
		var shadeRect = d3.select('g#shadedRect')
...
boxer(payNest,startDate,recessions);
	...
	d3.selectAll('div#timeRadio input').on('change',function(d){
	...
	d3.selectAll('div#sortRadio input').on('change',function(d){
	...
	d3.selectAll('div#shadeRadio input').on('change',function(d){
		shadeCriteria = this.value;
		console.log(this.value)
		console.log(shadeCriteria)
		boxer(payNest, startDate, recessions);
		})
