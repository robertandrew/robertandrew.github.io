...
function pricer(startDate, sortDirection) {
	...
	//Filters out early data
	thisSet = prices.filter(function(d){return d.date >= startDate });
	console.log(thisSet.length);	
	console.log(thisSet[0].date);
	
	//Calculates change since startDate
		//For each item, looks for a match on series ID and date for both the start date and today's	
