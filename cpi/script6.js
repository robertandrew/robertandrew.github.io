	...
	if(sortDirection == 'ascending'){	
		...
		//label the columns for each category
			...
			var columnLabels = [{"label" : "Item", 
										...
										"ident" : "endLabel",
										"width" : thisW / 8,},
										{"label" : "Change in price since " + startLabel,
										"ident" : "changeLabel",
										"width" : thisW / 2,}];
		console.log(thisSet);	
		//Set regular widths for each column, starting at zero and ending at w/2
			...
			var scaleColumns = d3.scale.ordinal()
				...
				.rangePoints([0,thisW/2]);
			
					z
			//Add divs for each column
			d3.select('div#PRICE div.chartContainer')
				.append('g').attr('id','columns')//id just for DOM purposes
				.selectAll('div')
					
			
			
			var columns = thisG.selectAll('g')
				.data(columnLabels)
				.enter()
				.append('g')
				.attr('class','columns')//right now it's only for use as a selector
				.attr('transform',function(d,i){return 'translate(' + scaleColumns(d.ident) + ',0)' });		
						
			d3.selectAll('g.columns text')
				.remove();
