...
function liner(startDate, otherCode) {
//Filters out select set into temporary working subset
//Calculates YoY change for All Items and select dataset
//Sets the domain and range for the scales based on available sets
//Selects/resizes the SVG and calls both scales
...
//Updates the coverage of the shaded rect 
		...
		otherCode = iSerieser(otherCode);
		
		//filter out the live datasets and set their values
		var cpiSet = indexes.filter(function(d){return d.series_id.trim() == cpiCode});
		var otherSet = indexes.filter(function(d){return d.series_id.trim() == otherCode});		
		
		cpiSet.forEach(function(d){d.value = +d.value});
		otherSet.forEach(function(d){d.value = +d.value});
		
		console.log(cpiSet);
		console.log(otherSet);

		//set scales based on the combination of the two datasets
		...
		//Set scales, domains and ranges 
		var cpiExtent = d3.extent(cpiSet, function(d){return d.value});
		var otherExtent = d3.extent(otherSet, function(d){return d.value});		
		var fullExtent = [Math.min(cpiExtent[0],otherExtent[0]) , Math.max(cpiExtent[1],otherExtent[1])];
		
		var cpiDateExtent = d3.extent(cpiSet, function(d){return d.date});		
		var otherDateExtent = d3.extent(otherSet, function(d){return d.date});
							...
							d3.tsv('pricePeriods.tsv', function(error3, data3) {
								d3.tsv('priceValues.tsv', function(error4, data4) {
									...
									iSerieser.domain(iData2.map(function(d){
										...
										.range(iData2.map(function(d){return d.series_id.trim()}));									
